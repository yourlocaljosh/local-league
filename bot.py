...
async def stats(interaction: discord.Interaction, user: discord.Member = None):
    ...
    idx = next((i for i, (uid, _) in enumerate(
        ranked) if int(uid) == user.id), None)
    if idx is None:
    ...
    embed.add_field(name="Record",
                    value=f"{user_stats['wins']}W - {user_stats['losses']}L",
                    inline=True)
    ...
    if idx < len(ranked) - 1:
        ...
        neighbor_lines.append(
            f"{idx + 2}. {below_user.display_name} — {below_stats['elo']}")
    ...
    if medals:
        ...
        medals_text = "\n".join(
            f"{emoji_map.get(m['medal'], '🏅')} {m['title']}" for m in medals)
    else:
...
@tree.command(name="leaderboard",
              description="View the current top 10 ELO leaderboard")
...
async def match(
    ...
    if set_count and winner_sets and loser_sets:
        ...
        if len(w_list) == set_count == len(l_list):
            for i in range(set_count):
                set_lines.append(
                    f"> Set {i + 1}: {w_list[i].strip()}-{l_list[i].strip()}")

    base = result['elo_gain']
    bonus = result['bonus']
    bonus_text = f" (+{bonus} bonus)" if bonus else ""
    winner_line = f"> Winner **+{base}**{bonus_text} ({
        w_stats['wins']}W {
        w_stats['losses']}L)"
    loser_line = f"> Loser **-{
        result['elo_loss']}** ({
    ...
    rec = w_stats.get('head_to_head', {}).get(
        str(loser.id), {'wins': 0, 'losses': 0})
    w_new = w_stats['elo']
    l_new = l_stats['elo']
    msg += (
...
@tree.command(name="medal",
              description="Grant a tournament medal to a player (admin)")
@app_commands.describe(user="The player to grant the medal to",
                       medal="Medal type: gold, silver, or third",
...
async def medal(
    ...
    if not is_admin(interaction.user):
        return await interaction.response.send_message("No permission", ephemeral=True)
    medal = medal.lower()
    if medal not in ["gold", "silver", "third"]:
        return await interaction.response.send_message("Medal must be gold, silver, or third.", ephemeral=True)
    emoji_map = {'gold': '🥇', 'silver': '🥈', 'third': '🥉'}
    medal_emoji = emoji_map[medal]
    data = load_data()
    register_user(data, user.id)
    data[str(user.id)].setdefault('medals', []).append(
        {"medal": medal, "title": title})
    save_data(data)
    await interaction.response.send_message(f"{medal_emoji} **{user.display_name}** awarded **{medal.upper()}** for *{title}*")


@tree.command(name="h2h",
              description="View head-to-head record between two players")
@app_commands.describe(player1="First player (mention)",
                       player2="Second player (mention)")
async def h2h(
        interaction: discord.Interaction,
        player1: discord.Member,
        player2: discord.Member):
    if player1.id == player2.id:
        return await interaction.response.send_message("You must specify two different players.", ephemeral=True)
    data = load_data()
    register_user(data, player1.id)
    register_user(data, player2.id)
    s1 = get_stats(data, player1.id)
    s2 = get_stats(data, player2.id)
    e1, e2 = s1['elo'], s2['elo']
    ranked = sorted(data.items(), key=lambda x: x[1]['elo'], reverse=True)

    def find_rank(uid): return next(
        (i + 1 for i, (u, _) in enumerate(ranked) if int(u) == uid), '-')
    r1, r2 = find_rank(player1.id), find_rank(player2.id)
    rec = s1.get('head_to_head', {}).get(
        str(player2.id), {'wins': 0, 'losses': 0})
    total = rec['wins'] + rec['losses']
    msg = (
        f"**Head to Head**\n{r1}. {
            player1.display_name} ({e1}) vs. {r2}. {
            player2.display_name} ({e2})\n\n" f"Games: {total}\n{
...
@tree.command(name="setlosses",
              description="Set a player's loss count (admin only)")
async def setlosses(
        interaction: discord.Interaction,
        user: discord.Member,
        value: int):
...
@tree.command(name="setwins",
              description="Set a player's win count (admin only)")
async def setwins(
        interaction: discord.Interaction,
        user: discord.Member,
        value: int):
...
@tree.command(name="alltime",
              description="View the top 10 all-time Elo gainers")
...
async def modifyh2h(
    ...
    h2h1 = data[k1].setdefault(
        'head_to_head', {}).setdefault(
        k2, {
            'wins': 0, 'losses': 0})
    h2h2 = data[k2].setdefault(
        'head_to_head', {}).setdefault(
        k1, {
            'wins': 0, 'losses': 0})
...
@tree.command(name="rivals",
              description="Show the top 10 rivalries by most games played")
async def rivals(interaction: discord.Interaction):
    ...
    for uid, entry in data.items():
        for opp_id, rec in entry.get('head_to_head', {}).items():
            ...
            wins_a = data[str(a)]['head_to_head'].get(
                str(b), {}).get('wins', 0)
            wins_b = data[str(b)]['head_to_head'].get(
                str(a), {}).get('wins', 0)
            total = wins_a + wins_b
    ...
    for i, ((a, b), wins_a, wins_b, _) in enumerate(top, start=1):
        ...
        if wins_a >= wins_b:
            lines.append(
                f"{i}. {
                    user_a.display_name} ({elo_a}) {wins_a}W - {wins_b}W {
                    user_b.display_name} ({elo_b})")
        else:
            lines.append(
                f"{i}. {
                    user_b.display_name} ({elo_b}) {wins_b}W - {wins_a}W {
                    user_a.display_name} ({elo_a})")

    await interaction.followup.send("\n".join(lines))


@tree.command(name="history",
              description="Show last 10 matches and Elo trend for a player")
@app_commands.describe(
...
async def history(
    ...
    for h in history_list:
        ...
        score_str = f"{score_w}-{score_l}" if (
            score_w is not None and score_l is not None) else "?"
...
@tree.command(name="loghistory",
              description="Admin: retroactively add a match history entry (no Elo change)")
@app_commands.describe(
    winner="Winner",
    loser="Loser",
    score_w="Winner's score",
...
@tree.command(name="setpeak",
              description="Admin: set a player's peak ELO manually")
@app_commands.describe(
...
async def dstats(
    ...
    pos = next((i for i, (uid, _) in enumerate(
        ranked) if int(uid) == user.id), None)
...
def _restore_record_for_undo(rec, *, voided_by_id: int, reason: str):
    ...
    if live_rec.get("status") != "active":
        return False, f"That match is already {
            live_rec.get(
                'status', 'not active')}."
    ...
    return True, f"Voided match `{
        live_rec['match_id']}`. Stats restored to the pre-match snapshot."
...
async def _match_summary_line(interaction: discord.Interaction, rec) -> str:
    ...
    if kind == "singles":
        ...
        return f"`{match_id}` singles — {winner} {
            summary.get(
                'score_w',
                '?')}-{
            summary.get(
    ...
    if kind == "doubles":
        ...
        return f"`{match_id}` doubles — {w_names} {
            summary.get(
                'score_w',
                '?')}-{
            summary.get(
...
@tree.command(name="lastmatch",
              description="Show the most recent active match ID")
...
@tree.command(name="undo",
              description="Admin: undo the most recent active match")
...
@tree.command(name="voidmatch",
              description="Admin: void a match by match ID and restore stats")
@app_commands.describe(
...
@tree.command(name="editmatch",
              description="Admin: edit a singles match by voiding it and logging the corrected result")
@app_commands.describe(
...
@tree.command(name="deditmatch",
              description="Admin: edit a doubles match by voiding it and logging the corrected result")
@app_commands.describe(
